  ...
  for sub_dir in sub_dirs:
    ...
    for file_name in file_list:
      ...
      if percentage_hash < validation_percentage:
        ...
        testing_images.append(base_name) #added to evaluate all data in the end
        training_images.append(base_name) #added to train on all data
        ...
        testing_images.append(base_name) #added to evaluate all data in the end
    print(sub_dir, np.shape(training_images), np.shape(testing_images),np.shape(validation_images))
    result[label_name] = {
        'dir': dir_name,
...
def main(_):
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
      train_accuracy, cross_entropy_value = sess.run(
          ...
          feed_dict={bottleneck_input: train_bottlenecks,
                     ground_truth_input: train_ground_truth})
      print('%s: Step %d: Train accuracy = %.1f%% (N=%d)' % (datetime.now(), i,
                                                      train_accuracy * 100, len(train_bottlenecks)))
      print('%s: Step %d: Cross entropy = %f' % (datetime.now(), i,
                                                 cross_entropy_value))
      validation_bottlenecks, validation_ground_truth, valid_filenames = (
  ...
  np.savetxt(str(FLAGS.trainset_dir), train_filenames, fmt = '%s')
  np.savetxt(str(FLAGS.testset_dir), test_filenames, fmt = '%s')
  np.savetxt(str(FLAGS.validset_dir), valid_filenames, fmt = '%s')
  ...
  if FLAGS.print_misclassified_test_images:
    ...
    for i, test_filename in enumerate(test_filenames):
      temp2 = []
      temp2.append(test_filename)
      temp2.append(predictions[i])
      temp2.append(test_ground_truth[i].argmax())
      final_results.append(temp2)
      if predictions[i] != test_ground_truth[i].argmax():
        temp = []
        temp.append(test_filename)
        temp.append(list(image_lists.keys())[predictions[i]])
        xtest.append(temp)
        #print('%70s  %s' % (test_filename,
        #                    list(image_lists.keys())[predictions[i]]))
    np.savetxt(str(FLAGS.xtestset_dir), xtest, fmt = '%s')
  ...
  #print(str(FLAGS.trainset_dir))
  #a = np.nonzero(test_ground_truth)
  #b = np.nonzero(predictions)
  #print(type(a), np.shape(a), type(b), np.shape(b), type(predictions), np.shape(predictions), predictions)
  #final_results = np.transpose(np.vstack((a,predictions)))
  #print(final_results)
  #final_results[:,0] = test_ground_truth
  #final_results[:,1] = predictions
  #np.savetxt(str(FLAGS.prediction_dir), final_results, fmt = '%s')
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--trainset_dir',
      type=str,
      default='/tmp/trainset_dir',
      help='How many images to train on at a time.'
  )
  parser.add_argument(
      '--validset_dir',
      type=str,
      default='/tmp/validset_dir',
      help='How many images to train on at a time.'
  )
  parser.add_argument(
      '--testset_dir',
      type=str,
      default='/tmp/testset_dir',
      help='How many images to train on at a time.'
  )
  parser.add_argument(
      '--prediction_dir',
